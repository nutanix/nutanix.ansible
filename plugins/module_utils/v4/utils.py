...
def strip_read_only_fields(spec, extra_fields=None):
    ...
    of read-only fields (create_time, update_time, created_by, updated_by,
    ext_id, links, tenant_id, created_timestamp, modified_timestamp,
    is_system_defined, is_default). Modules whose entity has additional
    read-only fields (e.g. counters populated by the platform) can pass
    them via ``extra_fields``.
