...
class ProjectsInternal(Prism):
    ...
    def _build_spec_user_and_user_groups_list(self, payload, role_mappings):
        ...
        for role_mapping in role_mappings:
            if role_mapping.get("user") and not role_mapping["user"].get("uuid"):
                ...
                user["operation"] = "ADD"
                users.append(user)
                name = role_mapping["user"].get("username") or role_mapping["user"].get("principal_name")
                name_uuid_map[name] = user["metadata"]["uuid"]

            if role_mapping.get("user_group") and not role_mapping["user_group"].get("uuid"):
                user_group, err = _user_group.get_spec(params = role_mapping["user_group"])
                if err:
                    return None, err
                ...
                name = role_mapping.get("distinguished_name") or  role_mapping.get("idp", {}).get("group_name")
    ...
    def _build_spec_role_mappings(self, payload, role_mappings):
        ...
        payload["spec"]["project_detail"]["resources"]["external_user_group_reference_list"] = []
        ...
        for role_mapping in role_mappings:
            ...
            if role_mapping.get("user"):
                ...
                if role_mapping["user_group"].get("uuid"):
                    ...
                    payload["spec"]["project_detail"]["resources"]["external_user_group_reference_list"].append(user_group_ref)
        ...
        for role_uuid, val in role_user_groups_map.items():
            ...
            acp["acp"]["resources"]["filter_list"], err = _acp.build_role_based_filter_list(acp["acp"]["resources"]["role_reference"], [self.project_uuid], cluster_uuids, collab)
            if err:
                return None, err
