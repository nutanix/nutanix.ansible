...
def get_module_spec():
    license_key_mappings_spec = dict(
        key=dict(type="str"),
        quantity_used=dict(type="double"),
    )
    license_key_assignments_spec = dict(
        cluster_ext_id=dict(type="str"),
        license_key_mappings=dict(
            type="list",
            elements="dict",
            options=license_key_mappings_spec,
            obj=licensing_sdk.LicenseKeyMapping,
        ),
    )
    module_args = dict(
        ext_id=dict(type="str"),
        state=dict(type="str", choices=["present"], default="present"),
        license_key_assignments=dict(
            type="list",
            elements="dict",
            options=license_key_assignments_spec,
            obj=licensing_sdk.LicenseKeyAssignment,
            required=True,
        ),
    )
    return module_args
...
def assign_license_keys_to_cluster(module, licensing_keys, result):
    sg = SpecGenerator(module)
    default_spec = licensing_sdk.LicenseKeyAssignmentSpec()
    spec, err = sg.generate_spec(obj=default_spec)
    if err:
        result["error"] = err
        module.fail_json(
            msg="Failed generating assign license keys to cluster Spec", **result
        )
    ...
    try:
        resp = licensing_keys.assign_license_keys_to_cluster(body=spec)
