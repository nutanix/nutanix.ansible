...
DOCUMENTATION = r"""
---
module: ntnx_cluster_disk_stats_info_v2
short_description: Retrieve DiskStat information from Nutanix Prism Central
version_added: 2.7.0
description:
  - This module allows you to fetch information about DiskStat in Nutanix Prism Central.
  - Fetches performance statistics for a specific physical disk identified by C(ext_id).
  - The Nutanix v4 DiskStat API only supports single-disk lookup, so C(ext_id) is required.
...
EXAMPLES = r"""
- name: Fetch disk stats info during a time interval
...
- name: Fetch disk stats info with all attributes
  nutanix.ncp.ntnx_cluster_disk_stats_info_v2:
    nutanix_host: "{{ ip }}"
    nutanix_username: "{{ username }}"
    nutanix_password: "{{ password }}"
...
response:
  description:
    ...
    - Returns the single DiskStat identified by external ID (with its time-series metrics).
  returned: always
  type: dict
  sample:
    {
...
ext_id:
  description:
    - The external ID of the disk for which stats were fetched.
...
def get_disk_stats_info(module, result, api_instance):
    ...
    if getattr(resp, "data", None):
        ...
        module.fail_json(
            msg="Failed fetching disk stats info for ext_id: {0}".format(ext_id),
            **result,
        )
...
def run_module():
    ...
    get_disk_stats_info(module, result, api_instance)
