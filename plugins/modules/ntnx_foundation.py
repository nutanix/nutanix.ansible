...
from ansible_collections.nutanix.ncp.plugins.module_utils.foundation.image_nodes import (  # noqa: E402
    ImageNodes,
)
from ansible_collections.nutanix.ncp.plugins.module_utils.foundation.progress import (  # noqa: E402
    Progress,
)
...
def get_module_spec():
    ...
    discovery_mode_node_spec=dict(
      ...
      image_now=dict(type="bool", required=True),
      discovery_override=dict(type="dict",required=False,options=discovery_override),
      ipmi_password=dict(type="str", required=False, no_log=True),
      ipmi_user=dict(type="str", required=False),
    )
    ...
    module_args = dict(
        ...
        foundation_central=dict(type="dict",required=False, options=foundation_central),
...
def run_module():
    module = FoundationBaseModule(
        ...
        required_if=[
          ("bond_mode", "dynamic",("bond_lacp_rate",))
        ],
        include_wait=True
    )
