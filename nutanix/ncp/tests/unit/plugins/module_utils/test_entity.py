...
from ansible_collections.nutanix.ncp.tests.unit.plugins.modules.utils import AnsibleExitJson, ModuleTestCase
...
class TestEntity(ModuleTestCase):
    ...
    def test_validate_request(self):
        response = self.builder.validate_request(self.builder.module,
                                                 'a218f559-0ec0-46d8-a876-38f7d8950098',
                                                 self.builder.netloc, 1)
        self.assertEqual(response.get("status"), 'succeeded')

    def test_negative_validate_request(self):
        exception = None
        try:
            self.builder.validate_request(self.builder.module,
                                          'wrong_uuid',
                                          self.builder.netloc, 1)
        except Exception as e:
            exception = e
